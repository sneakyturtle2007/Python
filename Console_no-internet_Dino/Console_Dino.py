...
class Dino():
    def __init__(self,cact):
        self.image = tk.PhotoImage(file="C:\\Users\\Luis Blake\\Desktop\\Console_no-internet_Dino\\dinosaur.gif")
...
class Cactus:
    def __init__(self):
        self.image = tk.PhotoImage(file="C:\\Users\\Luis Blake\\Desktop\\Console_no-internet_Dino\\cactus.png")
        self.X_cor = 490
    ...
    def Cactus_Move(self,dino):
        if not self.lost: 
            self.X_cor -= 5
            canvas.move(self.imageobject,-5,0)
            if(dino.get_X() == self.X_cor and dino.get_Y() >= (250 - self.image.height())):
                canvas.delete("all")
...
window.after(100,littlecactus.Cactus_Move,littledino)
window.mainloop()
